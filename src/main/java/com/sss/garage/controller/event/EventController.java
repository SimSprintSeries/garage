...
public class EventController extends SssBaseController {
    ...
    public Page<EventDTO> getAllEvents(@Parameter(description = "The current result page requested") @RequestParam(value = "currentPage", defaultValue = DEFAULT_CURRENT_PAGE) final int currentPage,
                                       ...
                                       @Parameter(description = "Sorting method applied to the returned results") @RequestParam(value = "sort", defaultValue = "startDate") String sort,
        ...
        return this.eventFacade.getAllEvents(leagueId, trackId, completed, pageable).map(e -> mapper.map(e, EventDTO.class));
    }
